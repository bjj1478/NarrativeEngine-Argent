                ...
                await saveCampaignState(campaign.id, {
                    context: {
                        ...
                        surpriseConfig: {
                            ...
                            types: seeds.surpriseTypes.length >= 3 ? seeds.surpriseTypes : (ctx.surpriseConfig?.types ?? []),
                            tones: seeds.surpriseTones.length >= 3 ? seeds.surpriseTones : (ctx.surpriseConfig?.tones ?? []),
                        },
                        encounterConfig: {
                            initialDC: ctx.encounterConfig?.initialDC ?? 198,
                            dcReduction: ctx.encounterConfig?.dcReduction ?? 2,
                            types: seeds.encounterTypes.length >= 3 ? seeds.encounterTypes : (ctx.encounterConfig?.types ?? []),
                            tones: seeds.encounterTones.length >= 3 ? seeds.encounterTones : (ctx.encounterConfig?.tones ?? []),
                        },
                        worldEventConfig: {
                            initialDC: ctx.worldEventConfig?.initialDC ?? 498,
                            dcReduction: ctx.worldEventConfig?.dcReduction ?? 2,
                            who: seeds.worldWho.length >= 3 ? seeds.worldWho : (ctx.worldEventConfig?.who ?? []),
                            where: seeds.worldWhere.length >= 3 ? seeds.worldWhere : (ctx.worldEventConfig?.where ?? []),
                            why: seeds.worldWhy.length >= 3 ? seeds.worldWhy : (ctx.worldEventConfig?.why ?? []),
                            what: seeds.worldWhat.length >= 3 ? seeds.worldWhat : (ctx.worldEventConfig?.what ?? []),
                        },
                        npcIntroConfig: {
