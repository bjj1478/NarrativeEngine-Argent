 ...
 * 100 always Fumbles. Adjacent values sharing a label are coalesced, so a
 * rating yields ~20 bands rather than 100, and the result tiles 1..100 exactly
 * with no gap or overlap — which is what `validateBands` demands.
 */
 ...
 * resolving the way it always has. New campaigns never see this — see
 * `buildDefaultDieTypes` below.
 */
 ...
 * LEGACY set — never on the current default, which would change how an
 * in-progress campaign resolves. `diceConfig`, if present, supplies its
 * thresholds.
 *
 * This previously reached for `dt_d20` inside `buildDefaultDiceSystem()`, which
 * silently became a no-op the moment the defaults stopped containing one.
 */
